...
vector<double> getXY(double s, double d, const vector<double> &maps_s, const vector<double> &maps_x, const vector<double> &maps_y,vector<double> &maps_dx,vector<double> &maps_dy)
{
  ...
  // vector<double> pts_sp_x;
  // vector<double> pts_sp_y;
  // vector<double> pts_sp_s;
  // vector<double> pts_sp_dx;
  // vector<double> pts_sp_dy;

  // int wp3 = wp2+1  ;
  // // int wp3 = ( (wp2+1)%maps_x.size() );

  // vector<int> waypt_sp = {prev_wp,wp2,wp3 };

  // for(int i=0; i<waypt_sp.size(); ++i){
  //   pts_sp_s.push_back(maps_s[i]);
  //   pts_sp_dx.push_back(maps_dx[i]);
  //   pts_sp_dy.push_back(maps_dy[i]);

  //   pts_sp_x.push_back(maps_x[i]);
  //   pts_sp_y.push_back(maps_y[i]);


  // }

  // spline_x_s.set_points(pts_sp_s,pts_sp_x);
  // spline_y_s.set_points(pts_sp_s,pts_sp_y);
  // spline_dx_s.set_points(pts_sp_s,pts_sp_dx);
  // spline_dy_s.set_points(pts_sp_s,pts_sp_dy);


  // double x = spline_x_s(s) + d * spline_dx_s(s);
  // double y = spline_y_s(s) + d * spline_dy_s(s);

  // cout<< "xold: "<< x<< " y_old: "<<y<< " x_spline: "<< x_new<< " y_spline: "<<y_new<< endl;
...
vector<vector<double> > generate_ego_trajectory(Vehicle& ego,int horizon,int lane, const vector<double> &map_waypoints_s, const vector<double> &map_waypoints_x, const vector<double> &map_waypoints_y,const vector<double> &previous_path_x,const vector<double> &previous_path_y,vector<double> &map_waypoints_dx,vector<double> &map_waypoints_dy){

  int prev_size = previous_path_x.size();

  // instead of taking all points, lets take only 25 points from the last rajectory.
  int const_num_points = 50;
  if (const_num_points<prev_size){
    prev_size = const_num_points;
  }

  // Use frenet coordinates, since the calculations will be simpler.
  vector<double> pts_s;
  vector<double> pts_d;
  ...
  // The simulator is sending all unused points.  

  double ref_s = ego.last_s; 
  double ref_d = ego.last_d;
  cout<< "\n ego.last_s = "<<ego.last_s<< " ego.last_d = "<< ego.last_d<< endl;
  cout<< " ego.vel = "<<ego.vel<< " prev_size = "<< prev_size<< " \n" <<endl;
  ...
  double car_d_prev;
    ...
    // cout<< "ref_vel= "<<ref_vel << "mph, in m/s="<< ref_vel/2.24 <<endl;
    // cout<< " car_s: "<<car_s<<", car_d: "<<car_d << ", car_x: "<<car_x<< ", car_y: "<<car_y<< ", horizon: "<< horizon<< ", car_yaw: " << car_yaw<< ", lane: "<<lane<< endl;


  car_s_prev = ego.last_s - (ego.delta_t * ego.vel);
  car_d_prev = ego.last_d;

  // Need to ensure prev value is not greater than ref_s
  if (car_s_prev>=ref_s){
    car_s_prev = ref_s - 0.01;
  }
  ...
  // not taking prev points, since the s values will not be monotonically increasing,
  //  since we already have pushed the ref_s onto the vector.
  for (int i = 0;i<3;++i){
  ...
  cout<< "Printing anchor points..." <<endl;
  ...
  vector<double> temp_sddata;

  cout<< "Entering adding prev points loop prev_data.size = "<< prev_size << endl;
  // Add the first 25 points from previous_path_x&y, so that we have some continuity.
  double angle;
  for(int i=0;i<prev_size;++i){
    
    int start_idx = horizon-prev_size;

    next_x_vals.push_back(ego.prev_traj_x[start_idx+i]);
    next_y_vals.push_back(ego.prev_traj_y[start_idx+i]);
    next_s_vals.push_back(ego.prev_traj_s[start_idx+i]);
    next_d_vals.push_back(ego.prev_traj_d[start_idx+i]);

  }
  cout<< "\nExiting adding prev points loop"<< endl;


  cout<< "Data after adding last prev_data points to the new traj..prev_data.size() = "<< prev_size << endl;
  cout<< "next_s_vals size: "<< next_s_vals.size()<< " " << endl;

  // cout<< "last s val: "<< temp_sddata[0]<< "last d val: " << temp_sddata[1]<< "ego.last_s="<< ego.last_s<< "ego.last_d: "<< ego.last_d<< endl;
  ...
  // s_temp = ref_s;
  new_s_point = ego.last_s;

  double tgt_s = 30;
  double tgt_d = s(new_s_point); 
  double tgt_distance = sqrt( tgt_s*tgt_s + tgt_d*tgt_d);
  double multiplier = tgt_s/tgt_distance;

  cout<< "multiplier: " << multiplier<< endl;

  cout<< "Entering horizon loop ... (horizon - prev)_size: "<< horizon-prev_size << endl;
  for (int i=0;i<(horizon-prev_size); ++i){

    new_s_point = ego.get_trajectory(new_s_point,multiplier);
    new_d_point = s(new_s_point);

    vector<double> newwaypt = getXY(new_s_point,new_d_point,map_waypoints_s,map_waypoints_x, map_waypoints_y,map_waypoints_dx,map_waypoints_dy);

    cout<< "idx: "<<i<<" new_s_point: "<<new_s_point<<" new_d: "<< new_d_point<< " x: "<< newwaypt[0]<< " y: "<< newwaypt[1] << endl;
  ...
  cout<< "Exiting horizon loop ... " << endl;
  // cout<< "next s size: " << next_s_vals.size() << "\n"<< endl;

  // Set the last values of s and d. 
  ego.last_s = new_s_point;
  ego.last_d = new_d_point;

  // cout<< "last recorded new_s_point: "<< ego.last_s<<"d point: "<<ego.last_d<< endl;

  ego.prev_traj_x = next_x_vals;
  ego.prev_traj_y = next_y_vals;
  ego.prev_traj_s = next_s_vals;
  ego.prev_traj_d = next_d_vals;
...
int main() {
  ...
  h.onMessage([&ego, &epoch,&myfile,&target_velocity,&current_state,&timestep,&horizon,&lane,&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            if (ego.is_initialized==false){
              // Init the ego vehicle object
              ego.setVars(car_s,car_d,10.0,timestep);
              ego.is_initialized = true;
            }

            cout<< "ego.vel = " <<ego.vel<< endl;
            ...
            for (int i=0; i<next_x_vals.size();++i){
              cout<< "next_x_vals["<< i << "] = "<< next_x_vals[i]<< " " ;
              cout<< "next_y_vals["<< i << "] = "<< next_y_vals[i]<< " ";
              cout<< "next_s_vals["<< i << "] = "<< next_s_vals[i]<< " ";
              cout<< "next_d_vals["<< i << "] = "<< next_d_vals[i]<< " ";

              if(i!=0){
                double dxdt = (next_x_vals[i] -  next_x_vals[(i-1)]) / timestep ;
                double dsdt = (next_s_vals[i] -  next_s_vals[(i-1)]) / timestep ;              
                cout<< " dx/dt: "<< dxdt <<" ";
                cout<< " ds/dt: "<< dsdt <<" ";

                if(fabs(dxdt) > 25 || fabs(dsdt) > 25){
                  cout<< "speed error"<< " ";
                }
              }
              cout<< " "<< std::endl; 
