...
vector<vector<double> > generate_ego_trajectory(double car_d,double car_s,double ref_vel,int horizon,double car_x, double car_y,double car_yaw,int lane, const vector<double> &map_waypoints_s, const vector<double> &map_waypoints_x, const vector<double> &map_waypoints_y,const vector<double> &previous_path_x,const vector<double> &previous_path_y){
  ...
  if(prev_size<2){
    ...
    // Since we have lot of points on the previous_path_* ,
    //  we will just add more points after the last point of this data. This will enable smooth transitions.  
    ...
    // cout<< "ref_x = "<< ref_x<< " ref_y = "<<ref_y << " ref_x_prev = "<< ref_x_prev<<" ref_y_prev = "<< ref_y_prev<<endl;
    ...
    // Testing new way of calculating the anchor points. Not sure if this works. If not, delete these 4 lines.
    ref_s = car_s + prev_size*0.02*ref_vel/2.24; 
    ref_d = car_d;

    car_s_prev = ref_s - 0.02*ref_vel/2.24 ;
    car_d_prev = car_d;
  ...
  for(int i=0;i<prev_size;++i){
    ...
    // in order to get the s, d values of the previous path, calculate the heading angle
